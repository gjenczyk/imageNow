  ...
  function main ()
{
    try
    {
       debug = new iScriptDebug("deleteDoctypes", LOG_TO_FILE, DEBUG_LEVEL);
       debug.log("WARNING", "deleteDoctypes script started.\n");
       var listToFlush = new INDocTypeList("",DOC_TYPE_LIST);
       listToFlush.getInfo();
       docList = listToFlush.members;
       printf("%s\n",docList.length)
       for (var i = 0; i < docList.length; i++)
       {
          printf("%s - %s\n", docList[i].id, docList[i].name)
          var docDel = new INDocType(docList[i].id);
          if(!docDel.remove())
          {
            printf("Error - %i\n", docList[i].name);
          }
